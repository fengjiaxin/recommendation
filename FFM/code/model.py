...
class FFM(object):
    # config 是配置字典dict,里面包含关键信息
    def __init__(self,hp,feature2field_dict):
        ...
        # feature2field_dict :根据特征的维度获取该特征属于第几个field
        self.feature2field_dict = feature2field_dict
        # field 数量
        self.field_num = hp.field_num
    ...
    def inference(self):
        ...
        with tf.variable_scope('field_aware_interaction_layer'):
            v = tf.get_variable('v',shape=[self.feature_length,self.field_num,self.k],
                                     initializer = tf.truncated_normal_initializer(mean=0,stddev=0.01))

            # [None,num_classes]
            self.field_aware_interaction_terms = tf.constant(value=0.0,dtype=tf.float32)

            for i in range(self.feature_length):
                for j in range(self.feature_length):
                    # <v_i_fj,v_j_fi>
                    w_var = tf.reduce_sum(tf.multiply(v[i,self.feature2field_dict[j]],v[j,self.feature2field_dict[i]]))
                    # xi * xj
                    x_ij = tf.multiply(self.xs[:,i],self.xs[:,j])
                    self.field_aware_interaction_terms += tf.multiply(w_var,x_ij)
